...
class HUB75{
    ...
    void Column_Select(int ColumnNumber){
        if(Bit_Return(ColumnNumber,0)){gpio_put(Display_Pins[ 6],1);}else{gpio_put(Display_Pins[ 6],0);}
        if(Bit_Return(ColumnNumber,1)){gpio_put(Display_Pins[ 7],1);}else{gpio_put(Display_Pins[ 7],0);}
        if(Bit_Return(ColumnNumber,2)){gpio_put(Display_Pins[ 8],1);}else{gpio_put(Display_Pins[ 8],0);}
        if(Bit_Return(ColumnNumber,3)){gpio_put(Display_Pins[ 9],1);}else{gpio_put(Display_Pins[ 9],0);}
        if(Bit_Return(ColumnNumber,4)){gpio_put(Display_Pins[10],1);}else{gpio_put(Display_Pins[10],0);}
    }
    ...
    void drive_HUB75() {    // Not fully optimised
        ...
        for(int i = 1; i <= LED_Matrix_Buffer; i++){
            ...
            for(int ii = 1; ii <= scan; ii++){
                gpio_put((Display_Pins[11]),0);
                gpio_put((Display_Pins[11]),1);
                gpio_put((Display_Pins[12]),0);

                for(int ii = 1; ii <= LED_Matrix_Width*LED_Matrix_Panel_Count; ii++) {
                    // REG_WRITE(GPIO_OUT_W1TS_REG, (1 << 23));  // Set the pin
                
                    // ESP32 Compatible
                    if(1 <= ((Array[cycle ] & RBitMask)>>(Colour_Space[1] + Colour_Space[2]))-RColourSub*CurrentBuff){gpio_put((Display_Pins[0]),1);}else{gpio_put((Display_Pins[0]),0);}
                    if(1 <= ((Array[cycle ] & GBitMask)>>(Colour_Space[2])                  )-GColourSub*CurrentBuff){gpio_put((Display_Pins[1]),1);}else{gpio_put((Display_Pins[1]),0);}
                    if(1 <= ((Array[cycle ] & BBitMask)                                     )-BColourSub*CurrentBuff){gpio_put((Display_Pins[2]),1);}else{gpio_put((Display_Pins[2]),0);}
                    if(1 <= ((Array[cycle2] & RBitMask)>>(Colour_Space[1] + Colour_Space[2]))-RColourSub*CurrentBuff){gpio_put((Display_Pins[3]),1);}else{gpio_put((Display_Pins[3]),0);}
                    if(1 <= ((Array[cycle2] & GBitMask)>>(Colour_Space[2])                  )-GColourSub*CurrentBuff){gpio_put((Display_Pins[4]),1);}else{gpio_put((Display_Pins[4]),0);}
                    if(1 <= ((Array[cycle2] & BBitMask)                                     )-BColourSub*CurrentBuff){gpio_put((Display_Pins[5]),1);}else{gpio_put((Display_Pins[5]),0);}
                    ...
                    gpio_put((Display_Pins[13]),0);
                    gpio_put((Display_Pins[13]),1);
