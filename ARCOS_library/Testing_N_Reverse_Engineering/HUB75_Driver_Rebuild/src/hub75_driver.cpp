...
HUB75Driver::HUB75Driver()
  ...
  , initialized(false)
  , running(false)
  , current_brightness(128)  // Default to 50% brightness
...
bool HUB75Driver::init(const HUB75Config& cfg){
  ...
  /** Calculate LSB/MSB transition bit for BCM brightness control */
  lsb_msb_transition_bit = 0;  // For standard 5-bit BCM
...
bool HUB75Driver::start(){
  ...
  /** Apply initial brightness to the buffer */
  applyBrightnessToBuffer(backBuffer);
...
void HUB75Driver::show(){
  ...
  /** Apply brightness control to the newly converted buffer */
  applyBrightnessToBuffer(backBuffer);
...
/** Brightness control implementation - matches ESP32-HUB75 library approach */
void HUB75Driver::setBrightness(uint8_t brightness){
  if(!initialized){
    ESP_LOGW(TAG, "Driver not initialized - brightness change deferred");
  ...
  /** Apply brightness to both buffers */
  applyBrightnessToBuffer(frontBuffer);
  applyBrightnessToBuffer(backBuffer);
  
  ESP_LOGI(TAG, "Brightness set to %d/255 (%.1f%%)", brightness, (brightness * 100.0f) / 255.0f);
}

void HUB75Driver::applyBrightnessToBuffer(uint16_t* buffer){
  if(!buffer || !initialized){
    return;
  }
  
  const int hub75_rows = config.matrix_height / 2;
  const uint8_t blank = 1;  // Latch blanking
  const uint8_t brt = current_brightness;
  
  /** Iterate through all color planes and rows */
  for(int plane = 0; plane < config.colour_depth; plane++){
    /** Calculate brightness in pixels for this bit plane using ESP32-HUB75 algorithm */
    int bitplane = (2 * config.colour_depth - plane) % config.colour_depth;
    int bitshift = (config.colour_depth - lsb_msb_transition_bit - 1) >> 1;
    int rightshift = std::max(bitplane - bitshift - 2, 0);
    
    /** Calculate OE disable period by brightness (matches original library) */
    int brightness_in_x_pixels = ((config.matrix_width - blank) * brt) >> (7 + rightshift);
    ...
    int x_coord_max = (config.matrix_width + brightness_in_x_pixels + 1) >> 1;
    int x_coord_min = (config.matrix_width - brightness_in_x_pixels + 0) >> 1;
    
    /** Process each row in this color plane */
    for(int row = 0; row < hub75_rows; row++){
      int buffer_offset = (plane * hub75_rows * config.matrix_width) + (row * config.matrix_width);
      
      /** Set OE bits based on brightness calculation */
      for(int col = 0; col < config.matrix_width; col++){
        uint16_t* sample = &buffer[buffer_offset + col];
        
        /** Check if this pixel should have OE enabled (LEDs on) */
        if(col >= x_coord_min && col < x_coord_max){
          /** Enable output - clear OE bit (active low) */
          *sample &= ~(1 << OE_BIT);
          if(config.pins.oe_pin2 >= 0){
            *sample &= ~(1 << OE2_BIT);
          }
        } else {
          /** Disable output - set OE bit (LEDs off) */
          *sample |= (1 << OE_BIT);
          if(config.pins.oe_pin2 >= 0){
            *sample |= (1 << OE2_BIT);
          }
